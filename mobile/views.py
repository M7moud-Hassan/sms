...
from drivers.models import Drivers, Treasury, MMission, SMission, PMission
...
def index(request):
    if request.method == "GET":
        drivers = Drivers.objects.filter(status="Available")
        ...
        if serializer.is_valid():
            ...
            driver = Drivers.objects.filter(Q(status="Available") & Q(username=username) & Q(password=password)).first()
...
def export_invoice(request, phone, driver_name, vehicle_number):
    ...
    if vouchers:
        ...
        for voucher in vouchers:
            ...
            if item:
                ...
                item.save()
                mission = get_object_or_404(MMission, id=item.mmission.id)
                pmission = PMission.objects.filter(Q(mmission__id=mission.id) & Q(car_num=voucher.car_num)).first()
                if pmission:
                ...
                new_invoice.dinar = int(mission.cost)
                new_invoice.fils = int(round((mission.cost - int(mission.cost)) * 100))
                amount = mission.cost
                new_invoice.tamount = amount
                new_invoice.samount = mission.scost
                new_invoice.save()
            description = f"نقل مركبة للعميل نوع {voucher.car_mark} لون {voucher.car_color} من {from_location} إلى {to_location}"
            description = f"\u202B{description}\u202C"
            adinar = udinar = int(amount / items_count)
            afils = ufils = int(round(((amount / items_count) - udinar) * 100))
    ...
    sub_customer = ""
    if customer.notes:
        if "الشرك" in customer.notes and "المركزي" in customer.notes and "تجار" in customer.notes:
    ...
    try:
        ...
        # Kill any existing LibreOffice processes before starting a new one
...
def get_oil(request, driver_name, vehicle_number):
    ...
    if request.method == "GET":
        ...
        if exoil and Decimal(request.data['vehicle_meter']) < exoil.vehicle_meter + 1000:
            return Response({
                'error': 'The meter difference must be more than or equal to 1000 kms',
        ...
        if serializer.is_valid():
            ...
            if request.data.get('air_filter') in [True, 'true', 'True', 'yes', 'Yes', 1, '1']:
                vehicle.air_filter_meter = request.data['vehicle_meter']
                vehicle.current_air_filter_diff = 0.0
            if request.data.get('diesel_filter') in [True, 'true', 'True', 'yes', 'Yes', 1, '1']:
        ...
        if Decimal(data['vehicle_meter']) < vehicle.fuel_meter:
            return Response({'error': 'The meter difference must be greater than the last fuel meter'}, status=status.HTTP_400_BAD_REQUEST)
        if Decimal(data['vehicle_meter']) < vehicle.oil_meter + 1000:
            return Response({
                'error': 'The meter difference must be more than or equal to 1000 kms',
        ...
        if serializer.is_valid():
            ...
            vehicle.current_oil_diff = 0.0
            if data.get('air_filter') in [True, 'true', 'True', 'yes', 'Yes', 1, '1']:
                vehicle.air_filter_meter = data['vehicle_meter']
                vehicle.current_air_filter_diff = 0.0
            if data.get('diesel_filter') in [True, 'true', 'True', 'yes', 'Yes', 1, '1']:
